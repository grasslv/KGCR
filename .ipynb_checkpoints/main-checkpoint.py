...
import torch
import random
from Dataset import TrainDataset, data_load, KGDataset
from KGCR import *
...
if __name__ == '__main__':
    ...
    for epoch in range(num_epoch):
        ...
        if test_result[1] > max_recall:
            ...
            test_result10 = full_ranking(epoch, model, test_data, user_item_dict_train, None, False, step, 10, model_name, 'Test/', writer)
            test_result20 = full_ranking(epoch, model, test_data, user_item_dict_train, None, False, step, 20, model_name, 'Test/', writer)

            with open('./datasets/'+data_path+'/result_{0}.txt'.format(save_file_name), 'a') as save_file:
                save_file.write(str(args))
                save_file.write('\n epoch:'+str(epoch))
                save_file.write('\r\n-----------Test Precition@5:{0:.4f} Recall@5:{1:.4f} NDCG@5:{2:.4f}-----------'.format(test_result5[0], test_result5[1], test_result5[2]))
                save_file.write('\r\n-----------Test Precition@10:{0:.4f} Recall@10:{1:.4f} NDCG@10:{2:.4f}-----------'.format(test_result10[0], test_result10[1], test_result10[2]))
                save_file.write('\r\n-----------Test Precition@20:{0:.4f} Recall@20:{1:.4f} NDCG@20:{2:.4f}-----------'.format(test_result20[0], test_result20[1], test_result20[2]))
                save_file.write('\r\n------Best Test Precition@20:{0:.4f} Recall@20:{1:.4f} NDCG@20:{2:.4f}-----------'.format(max_test_result[0], max_test_result[1], max_test_result[2]))
